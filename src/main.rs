...
fn main() {
    ...
    if args.len() != 6 {
        eprintln!(
            "Usage: {} FILE PIXELS UPPERLEFT LOWERRIGHT CONCURRENT",
            args[0]
        );
        eprintln!(
            "Example: {} mandel.png 1000x750 -1.20,0.35 -1,0.20 1",
            args[0]
        );
        std::process::exit(1);
    }
...
/// like `"400x600"` or `"1.0,0.5"`.
///
/// `s` should have the form <left><sep><right>, where
/// <sep> is the character given by the `separator` arg,
/// <left> and <right> are string that can be parsed by `T::from_str`
///
/// If `s` has the proper form, return `Some<(x,y)>`, else return `None`
...
/// number of iterations it took for `c` to leave the circle of
/// radius 2 centered on teh origin.
///
...
///
/// bounds` gives the width and height of the buffer, which holds
/// one grayscale pixel per byte.
///
/// `upper_left` and `lower_right` specify points on the complex plane
/// corresponding to the upper-left and lower-right corners of the buffer.
fn render(
    pixels: &mut [u8],
    bounds: (usize, usize),
    upper_left: Complex<f64>,
